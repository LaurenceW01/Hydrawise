...
def quick_start():
    ...
    """
    print("🚀 HYDRAWISE API QUICK START")
    print("=" * 40)
    ...
    try:
        ...
        print("✅ Successfully connected to Hydrawise API!")
        
        # Analyze the response to show basic info
        print("\n📊 YOUR HYDRAWISE SYSTEM:")
        print("-" * 30)
        ...
        if 'controllers' in customer_data:
            ...
            for controller in controllers:
                print(f"   📡 {controller.get('name', 'Unknown Controller')}")
        ...
        if 'relays' in customer_data:
            ...
            for zone in zones:
                ...
                print(f"   💧 {zone_name} (ID: {zone_id}, Type: {zone_type})")
        ...
        if 'relays' in status_data:
            ...
            print(f"\n🚿 Currently running: {len(running_zones)} zones")
            for zone in running_zones:
                print(f"   ▶️ {zone['name']} - {zone['time_left']} remaining")
            
            if not running_zones:
                print("   💤 No zones currently running")
            
            print(f"\n📅 Upcoming runs: {len(upcoming_zones)} zones scheduled")
            for zone in upcoming_zones[:3]:  # Show first 3
                print(f"   ⏰ {zone['name']} - {zone['next_run']}")
            
            if len(upcoming_zones) > 3:
                print(f"   ... and {len(upcoming_zones) - 3} more")
        
        # Try to detect flow meter
        print("\n💧 Checking for flow meter data...")
            ...
            if isinstance(data, dict):
                for key, value in data.items():
                    if any(indicator in key.lower() for indicator in flow_indicators):
                        print(f"   🔍 Found potential flow data: {prefix}{key} = {value}")
                        flow_found = True
        ...
        if not flow_found:
            print("   ℹ️ No flow meter data detected in basic responses")
            print("   💡 Flow data may be available through:")
        ...
        print(f"❌ Error connecting to Hydrawise API: {e}")
        print("\n🔧 Troubleshooting:")
