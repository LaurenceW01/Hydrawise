...
def collect_reported_data():
    ...
    print("📊 COLLECTING PREVIOUS DAY REPORTED RUNS")
    print("=" * 60)
    
    # Calculate previous day
    previous_day = date.today() - timedelta(days=1)
    print(f"📅 Target Date: {previous_day.strftime('%A, %B %d, %Y')}")
    print("🎯 Target: First 10 reported runs with popup data")
    print("=" * 60)
    ...
    try:
        ...
        if existing_count > 0:
            print(f"⚠️  Found {existing_count} existing reported runs for {previous_day}")
            ...
            if response in ['y', 'yes']:
                with sqlite3.connect(storage.db_path) as conn:
                    ...
                    conn.commit()
                    print(f"🗑️  Cleared {existing_count} existing runs")
            else:
                print("📊 Keeping existing data, cancelling collection")
                return True
        ...
        if all_actual_runs and len(all_actual_runs) > 10:
            print(f"📊 Found {len(all_actual_runs)} total runs, using first 10")
        ...
        if not actual_runs:
            print("❌ No reported runs found for the previous day")
            return False
            
        print(f"📊 Collected {len(actual_runs)} reported runs")
        
        # Display summary before storing
        print("\n📋 COLLECTED RUNS SUMMARY:")
        print("-" * 80)
        ...
        print(f"\n💾 Storing runs in database for {actual_collection_date}...")
        stored_count = storage.store_actual_runs_enhanced(actual_runs, actual_collection_date)
        
        print(f"✅ Successfully stored {stored_count}/{len(actual_runs)} reported runs")
        print(f"📅 Data stored for: {actual_collection_date}")
        
        return True
        
    except Exception as e:
        print(f"❌ Error collecting reported data: {e}")
        import traceback
        traceback.print_exc()
