...
def test_periodic_collection():
    ...
    try:
        ...
        print(f"🔄 Testing periodic collection for today")
        ...
        if result.success or (result.details and 'skipped' in result.details):
            print(f"\n✅ Periodic collection test passed!")
            ...
            print(f"\n❌ Periodic collection test failed!")
            return False
            
    except Exception as e:
        print(f"\n❌ Periodic collection test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_daily_collection():
    ...
    try:
        ...
        print(f"🌅 Testing daily collection (previous day + current day)")
        ...
        if result.success:
            ...
            print(f"\n❌ Daily collection test failed!")
            return False
            
    except Exception as e:
        print(f"\n❌ Daily collection test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    """Run all tests"""
    print("🚰 HYDRAWISE REPORTED RUNS COLLECTION SYSTEM TEST")
    print("=" * 70)
    print("This test validates all three operational modes:")
    print("1. 🌅 Daily Collection (previous day + current day)")
    print("2. 🔄 Periodic Collection (current day deltas)")
    print("3. 👤 Admin Override Collection (manual)")
    print("=" * 70)
