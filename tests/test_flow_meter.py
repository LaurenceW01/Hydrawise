...
def test_flow_meter():
    ...
    print("🧪 TESTING FLOW METER DETECTION - ZONE 1 (2419310)")
    print("=" * 60)
    ...
    try:
        ...
        import time
        print("\n⏳ Step 3: Waiting 20 seconds for zone to start...")
        time.sleep(20)
        
        print("📊 Getting status during zone operation...")
        running_data = explorer.get_status_schedule()
        running_flow = explorer.get_flow_meter_data(running_data)
        
        if running_flow:
            print("✅ Flow meter data during operation!")
            for sensor_id, data in running_flow.items():
                print(f"   Sensor {sensor_id}: Rate = {data.get('rate', 0)} GPM")
                print(f"   Timer: {data.get('timer', 0)} seconds")
        else:
            print("❌ No flow meter data during operation")
        
        # Step 4: Check zone status
        print(f"\n🌱 Step 4: Checking zone {zone_id} status...")
        ...
        if flow_data:
            ...
            if 'sensors' in status_data:
                print(f"\n🔍 DEBUG: Found {len(status_data['sensors'])} sensors:")
