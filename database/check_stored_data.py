...
def main():
    ...
    print("🔍 Checking Stored Schedule Data in Database")
    print("=" * 50)
    ...
    try:
        ...
        # Check today's data
        today = date.today()
        print(f"\n📅 Checking scheduled runs for TODAY ({today}):")
        ...
        if today_runs:
            ...
            for i, run in enumerate(today_runs, 1):
                ...
                status = "🌧️ Rain" if run['is_rain_cancelled'] else "☀️ Normal"
                
                print(f"   {zone_name:<35} {start_time:<10} {duration:<8} {status:<15}")
        
        # Check tomorrow's data
        tomorrow = date.today() + timedelta(days=1)
        print(f"\n📅 Checking scheduled runs for TOMORROW ({tomorrow}):")
        ...
        try:
            with sqlite3.connect(db.db_path) as conn:
                ...
                if all_runs:
                    print(f"   📋 Total records in scheduled_runs table: {len(all_runs)}")
                    print("   " + "-" * 90)
                    ...
                    print("   ❌ No records found in scheduled_runs table")
        ...
        if total_stored > 0:
            print(f"\n✅ SUCCESS: Your scheduled runs are stored in the 'scheduled_runs' table!")
            print(f"📍 Table location: {db.db_path} → scheduled_runs table")
        else:
            print(f"\n⚠️  No scheduled runs found. The data may not have been written successfully.")
