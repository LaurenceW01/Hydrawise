...
class IntelligentDataStorage(DatabaseManager):
    ...
    def store_actual_runs_enhanced(self, actual_runs: List, collection_date: date = None) -> int:
        """
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for run in actual_runs:
                try:
                    ...
                    stored_count += 1
                    logger.info(f"Stored actual run: {run.zone_name} at {run.start_time.strftime('%I:%M %p')} - {popup_analysis.get('actual_gallons', 0):.1f} gal")
